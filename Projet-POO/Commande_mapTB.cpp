...
namespace NS_Comp_Mappage {
	...
	String^ Commande_mapTB::Select3(void) {
		return "SELECT [article].[id_article], [Contenir].[quantite], [article].[nom_article], [article].[couleur], [article].[nature] FROM [Projet].[dbo].[article] INNER JOIN [Contenir] ON ([article].[id_article] = [Contenir].[id_article]);";
	}
	String^ Commande_mapTB::Select4(void) {
		return "SELECT SUM([Contenir].[quantite] * [article].[prix_TTC_]) FROM [article] INNER JOIN [Contenir] ON [article].[id_article]=[Contenir].[id_article]";
	}
